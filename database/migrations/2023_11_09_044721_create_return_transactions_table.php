<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('return_transactions', function (Blueprint $table) {
            $table->id();
            $table->date('date')->comment('mencatat tgl pengembalian mobil');
            $table->integer('total_day')->unsigned()->default(0)->comment('total hari pinjaman saat pengembalian dilakukan');
            $table->integer('rental_fee')->unsigned()->comment('biaya sewa / hari');
            $table->integer('total_payment')->unsigned()->comment('total biaya');
            $table->foreignId('transaction_id')->constrained('transactions');
            $table->foreignId('user_id')->constrained('users');

            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('return_transactions');
    }
};
